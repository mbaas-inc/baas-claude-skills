...
| 경로 | 성격 | 캐시 | 용도 |
|---|---|---|---|
| `/<prefix>/<version>/baas-react.js` (예: `/1.0.0/`) | **불변**(고정 버전) | 1년(immutable) | 롤백 대상·버전 고정 |
| `/<prefix>/<channel>/baas-react.js` (예: `/v1/`) | **가변 별칭**(v1 채널 = 1.x 최신 빌드) | 60s + 무효화 | **생성 앱이 참조** → CDN push로 자동 업데이트 |
