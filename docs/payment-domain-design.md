# 결제 단일 도메인 설계 (Payment as one domain)
...
## 10. 합의 필요 (open questions)
- **식별자 이름**: `order_no` 단일로 확정? (현재 store=order_no, reservation=order_id — 통일 대상)
- **정산 권위**: 웹훅 우선(서버 확정) + 클라 confirm backstop 구조로 갈지, 클라 confirm 유지할지.
- **커스텀 금액 권위**: 고정가(관리자 가격 소스 신설) vs 변동가(prepare 기록액)만 우선 지원할지.
