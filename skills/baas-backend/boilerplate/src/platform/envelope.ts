...
export interface RequestContext {
   ...
   * `accountId` 는 **null 인데 이 값만 true** 인 조합이 정상이다.
   *
   * **두 값은 독립이다 — 네 조합이 전부 가능하다.** 특히 `accountId` 가 있으면서 이 값도
   * true 인 경우가 흔하다: 사장님이 자기 서비스에 손님으로도 로그인한 상태다(플랫폼 세션과
   * 프로젝트 회원 세션이 별도 트랙이라 공존한다). 그러니 **상호배타로 가정하지 마라** —
   * "회원이면 관리자가 아니다" 같은 분기는 사장님이 자기 앱을 쓰는 순간 깨진다.
   *
   * | `accountId` | `isProjectOwner` | 누구 |
   * |---|---|---|
   * | null | false | 비로그인 손님 |
   * | 있음 | false | 로그인한 손님 |
   * | null | true  | 소유자(회원 가입은 안 한 상태) |
   * | 있음 | true  | 소유자가 손님으로도 로그인한 상태 |
   *
