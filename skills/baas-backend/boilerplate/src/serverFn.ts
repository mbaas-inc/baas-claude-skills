 ...
 * 생겼다. 그래서 도구가 추론할 수 없고, 사람이 읽어도 의도를 알 수 없다.
 *
 * | 값 | 플랫폼이 하는 일 |
 * |---|---|
 * | `public` | 아무것도 막지 않는다. 비로그인 포함 누구나 |
 * | `member` | 로그인하지 않았으면 401 |
 * | `owner`  | 프로젝트 소유자가 아니면 403 |
 *
 * 플랫폼은 **자기가 이미 아는 사실**(로그인 여부·소유자 여부)까지만 강제한다.
 */
export type ServerFnAccess = 'public' | 'member' | 'owner'

export interface ServerFnOptions {
  access: ServerFnAccess
  /**
   * 본문이 **역할·계층·자원 범위를 추가로 판정한다**는 선언.
   *
   * 매니저·직원 같은 역할은 플랫폼이 모르므로 코드가 정한다. 그런데 그것을 `access` 의
   * 한 값으로 두면(예전 `'custom'`) **양자택일이 된다** — 로그인 강제를 얻으면 인가가
   * 선언에서 사라지고, 인가를 선언하면 401 을 손으로 다시 써야 했다(2026-09-16 발견).
   *
   * 두 축은 독립이다. `access` 는 플랫폼 집행이고, 이 값은 **본문에 인가가 있다는 사실**이다.
   *
   * ```ts
   * // 로그인은 플랫폼이 막고, 지점 판정은 코드가 한다
   * export const listBranch = serverFn<I, O>(async (input, ctx) => {
   ...
   * ```
   *
   * 본문이 403 을 던지는데 이 선언이 없으면 추출기가 빌드를 세운다 — 선언만 보고는
   * 「로그인 회원 아무나」와 구분할 수 없기 때문이다.
   */
  authorizes?: true
}
