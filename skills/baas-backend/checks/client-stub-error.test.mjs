 ...
 * 타입 시스템에 보이지 않는다.
 *
 * 파급은 403 하나가 아니다. 계약표가 보존을 약속한 409(중복·정원)도 클라이언트에서
 * 500 과 구분되지 않는다 — **예외로 전파되는 순간** 선착순 마감이 서버 장애와 같은 화면이
 * 된다. (실측한 앱은 마감을 `{ status: 'sold_out' }` 처럼 정상 응답 필드로 돌려받아 이
 * 경로를 타지 않았다. 가이드가 권하는 방식이고, 그 권고를 따르지 않은 프로젝트가 걸린다.)
 * 보일러플레이트는 프로젝트로 복사된 뒤 자동 갱신되지 않으므로, 여기서 새면 이후 생성되는
 * 모든 프로젝트에 박힌다.
 */
