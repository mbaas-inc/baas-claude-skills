...
규칙이 읽고 쓰는 컬렉션은 **이 역할이 만든다** — 스키마·필드·접근을 `baas` CLI 로 직접
선언한다. 규칙과 그 규칙이 지배하는 데이터는 소유자가 하나여야 한다. 스펙만 적어 다른
역할에 넘기면 산문으로 번역됐다가 재현되면서 어긋난다.

그 컬렉션은 **`service` 만 선언한다.** `public`·`member`·`owner`·`ref_owner` 를 주지 마라 —
