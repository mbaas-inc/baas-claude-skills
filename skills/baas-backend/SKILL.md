...
- **레이아웃도 가른다.** 손님 셸(헤더·푸터) 안에 관리자 화면을 넣으면 관리 콘솔에 손님용
  네비게이션과 「로그인 / 회원가입」 버튼이 남아, 본문이 「사장님 계정으로 로그인」이라고
  안내하는 옆에서 헤더가 회원가입을 권한다(2026-09-17 실측). 규칙은 `baas-integration-sdk`
  스킬의 「관리자 화면은 셸을 분리한다」에 있다 — 화면을 쓰는 쪽이 그 스킬이다
...
#### 등록 절차를 사용자에게 **먼저 말해라** — 네가 유일한 등록 경로다

사용자에게는 키를 넣을 화면이 **없다.** 콘솔에도 스튜디오에도 시크릿 관리 UI 가 없고
`baas secret` 은 이 작업 환경 안에서만 돈다. 그래서 「플랫폼 설정에서 등록하세요」 같은
안내는 **갈 곳이 없는 지시**다(2026-09-17 실측: 실제로 그렇게 답해 사용자가 화면을 찾았다).

- 키가 필요하면 **네가 요청한다** — "키를 여기 붙여 주시면 `STOCK_API_KEY` 로 등록할게요"
- 등록 상태를 물으면 `baas secret list` 를 **실행해서** 이름과 시각을 보여준다. 확인하지 않고
  "등록된 게 없어요" 라고 단정하지 마라
- **결제(토스) 키는 여기 해당하지 않는다.** 플랫폼이 운영 전체에 대해 환경변수로 들고 있고
  프로젝트 시크릿 저장소에 없다 — 네가 등록할 수도 조회할 수도 없으므로, 그건 플랫폼
  운영자 몫이라고 **구분해서** 안내한다. 섞어서 말하면 사용자가 자기 API 키도 어딘가
  설정 화면에서 넣어야 하는 줄 안다
