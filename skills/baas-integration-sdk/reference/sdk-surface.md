...
- **결제 세션/주문은 "결제하기" 클릭 시점(`handle.requestPayment`)에 생성된다** — 위젯을 열기만 하고 이탈하면
  잔재가 없다(SDK가 그 시점에 백엔드 `start` 를 호출). 앱은 이 배선을 신경 쓸 필요 없이 `beginWidgetCheckout` →
  `handle.requestPayment` 만 호출한다.
...
//   → 결제수단/약관 위젯이 앱 DOM 에 렌더됨(w.amount). order_no/예약/세션은 결제 버튼 클릭 시 생성:
...
// → 클릭 시 SDK가 start(예약+세션 생성) 호출 후 결제 요청. 성공 시 successUrl 리다이렉트(paymentKey/amount 쿼리).
//   order_no/reserved_at/form_data 는 SDK가 sessionStorage 에 보관 → 복귀 페이지에서:
...
const w = await s.beginWidgetCheckout({ productId, quantity: qty,
  methodsSelector: "#toss-payment-methods", agreementSelector: "#toss-agreement", customerKey });
//    → 결제수단/약관 위젯이 앱 DOM 에 렌더됨(w.amount). order_no/주문/세션은 결제 버튼 클릭 시 생성.
// 2) 결제 버튼 클릭 시:
await w.requestPayment({ successUrl: `${location.origin}/checkout-success`,
  failUrl: `${location.origin}/checkout-fail`, orderName });
//    → 클릭 시 SDK가 start(주문+세션 생성) 호출 후 결제 요청. 성공 시 successUrl 리다이렉트(paymentKey/orderId/amount 쿼리).
