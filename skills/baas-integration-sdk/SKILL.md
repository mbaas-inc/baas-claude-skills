...
고정 기능(회원·게시판·설문·예약·스토어)이 커버 못 하는 **사용자 정의 데이터**는 동적 컬렉션을 쓴다.

- **기존 기능 우선**: FAQ/공지→게시판, 문의/연락처→recipient, 예약→reservation … 기존 기능이
  커버하면 컬렉션을 만들지 말 것(서버가 예약어 거부). 컬렉션은 미충족 신규 수요 전용.
- **공동 설계 흐름**: 요구 → (표현 범위·제약을 알고) **UI 설계 + 그에 맞는 스키마 구성** →
  `baas collection create/field add`로 스키마 생성 → `useCollection` 프리미티브로 UI 연결.
  UI는 요구에 맞춰 **직접 설계**한다(범용 자동 렌더 아님 — 그건 관리자 콘솔 몫).
