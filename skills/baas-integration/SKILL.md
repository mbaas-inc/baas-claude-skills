...
| 로그아웃 | 인증 쿠키 삭제 |
| 계정정보 조회 | 로그인된 사용자 정보 |
| 비밀번호 변경 | 현재 비밀번호 확인 후 변경 (SNS 계정 제외) |
...
|------------|---------------|--------------|
| 회원가입, 로그인, 로그아웃, 인증, 비밀번호 변경 | [references/account.md](references/account.md) | `templates/baas.ts` 또는 `react/useLogin.tsx`, `react/useChangePassword.tsx` 등 |
...
4. **비밀번호 변경** (`POST /account/profile/change-password`) - 인증 필수, SNS 계정 제외
5. **로그아웃** (`POST /account/logout`) - 쿠키 삭제
