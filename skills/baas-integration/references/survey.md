...
```typescript
{
  ...
  data: {
    items: Array<{
      ...
      share_code: string | null, // 공유 코드 (참고용)
      form_url: string | null,   // 서버가 제공하는 전체 참여 URL (prefix 포함). 그대로 사용
...
> **참여 URL**: `form_url`을 그대로 사용하세요. 서버가 prefix를 포함한 완전한 URL을 응답에 담아 반환합니다 (클라이언트가 prefix를 하드코딩하지 않음).
...
```typescript
{
  ...
  data: {
    ...
    form_url: string | null,         // 서버 제공 전체 참여 URL (prefix 포함)
...
// 2. 서버가 응답에 담아준 form_url을 그대로 사용 (prefix 포함 완전한 URL)
