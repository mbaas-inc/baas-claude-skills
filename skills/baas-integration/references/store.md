...
- **통신판매중개업 고지(푸터) 필수** — 아래 [필수 표시 규약](#필수-표시-규약--통신판매중개업-고지-푸터)을 반드시 렌더링하세요.

---

## 필수 표시 규약 — 통신판매중개업 고지 (푸터)

스토어를 제공하는 **모든 페이지 하단 푸터**에 아래 통신판매중개업자 정보와 고지를 **고정 표기**해야 합니다 (전자상거래법 제20조① 통신판매중개자 고지). 이는 판매자(셀러) 정보가 아니라 **플랫폼 운영사(통신판매중개업자)의 고정 정보**이며, API로 내려오지 않으니 아래 문구를 그대로 넣으세요. **개별 셀러(판매자) 신원정보는 노출하지 않습니다.**
...
- "당사자 아님" 고지 문구는 통신판매중개업 고시의 핵심이므로 **생략 불가**입니다.
- **「사업자정보확인」은 위 공정위 URL로 새 창 하이퍼링크**(`target="_blank" rel="noopener noreferrer"`)를 걸고, 앵커 텍스트는 `사업자정보확인`으로 렌더링합니다. `wrkr_no`는 사업자등록번호에서 하이픈을 뺀 값(`1288802089`)입니다. **신고번호 등 텍스트는 그대로 두고 링크를 덧붙이는 형태**입니다 — 표시 의무는 링크로 대체되지 않습니다.
- 위 값(통신판매업신고번호·주소 등)은 플랫폼 사업자 정보로 변경될 수 있습니다. 변경 시 이 스킬 문구를 갱신하세요.
- 결제(청약) 화면에서는 `GET /terms` 약관의 고지 조항과 함께, 푸터에도 상시 노출되도록 합니다.
